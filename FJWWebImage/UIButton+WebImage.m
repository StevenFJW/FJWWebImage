...
- (void)imageWithURLString:(NSString *)URLString Size:(CGSize)size placeholderImage:(UIImage *)placeholderImage completed:(ESWebImageCompleted)completedBlock; {
    ...
    if (!placeholderImage) {
        placeholderImage = [FJWWebImage placeholderWithSize:size];
    }
    
    [self sd_setImageWithURL:[FJWWebImage URLWithURLString:URLString imageSize:size] forState:UIControlStateNormal placeholderImage:placeholderImage completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
